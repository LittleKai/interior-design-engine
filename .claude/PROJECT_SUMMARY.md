...
| Multi-run layouts | Done | `src/core/model.js`, `src/renderers/svg-renderer.js`, `src/renderers/iso-renderer.js`, `src/__tests__/run-coords.test.mjs`, schema JSON | Top-level `runs[]` với `{id, origin:{x,z}, direction, modules}` cho L/U/island/galley. **Quy ước tọa độ (thống nhất 2026-07-03):** trong mọi run, module `x` = vị trí DỌC trục run tính từ origin, `z` = offset vuông góc từ tường; east=+x, west=-x, south=+z, north=-z (north cần origin.z ở đầu xa). Auto-offset theo thứ tự module khi không có x>0 nào. `normalizeModel()` convert legacy `modules[]` → default run, resolve về world coords (idempotent — item đã có `_runDirection` giữ nguyên). Backend `interiorModelGeometry.js` mirror cùng logic. |
...
| Gemini image-to-design pipeline | Done | `src/ai/image-analyzer.js`, `src/ui/upload-panel.js`, `src/ui/compare-slider.js`, `src/core/model.js`, backend `routes/interior.js` (+/analyze-image, +/generate-render), `middleware/interiorQuota.js`, models `InteriorAnalysis`/`InteriorRender`/`InteriorQuota` | Frontend: `analyzeImage(file, opts)` resize ≤1600px → presigned B2 upload → POST /analyze-image. Debug console logs are gated by `?debug=1` or `localStorage.ide:debug=1`; unsupported schema requests log as `[ide:ai] unsupported:`. `normalizeModel()` records `_validationWarnings` for invalid void/glass material semantics. Compare-slider web component (pointer drag + clip-path). Upload panel with drop zone + hints textarea + status. Backend: analyze flow has 24h cache via sha256(imageUrl+hints), robust JSON extraction before repair loop, repair loop max 2 retries, Gemini Flash 3 default → Pro 3.1 escalate when hints contain "complex" or override="pro". AI may return `meta.unsupportedRequests[]` for unsupported schema requests and is instructed not to misuse `glass-smoked` / `kind:"void"`. Generate-render endpoint validates modelJson, uploads viewBase64 to `interior-design/conditioning/`, persists InteriorRender record. **Image-gen upstream đã wire** qua `GoogleGenAI` (`INTERIOR_IMAGE_MODEL`, default `gemini-2.5-flash-image`) — render thật upload lên `interior-design/renders/`; khi thiếu API key/lỗi thì fallback trả conditioning URL + `meta.pending=true` + `fallbackReason`. Rate limit 5/24h/user via `interiorQuotaCheck` middleware (bypass for admin/mod, disable via `INTERIOR_QUOTA_ENABLED=false`). |
...
| Raw box color + per-module style.colors | Done | `src/core/box-resolver.js` (`colorFaces`), `src/template-engine/interpreter.js` (`normalizeColorOverrides`), `src/template-engine/color-tokens.js` | Raw box có `color` → faces shade từ màu đó. Template modules nhận `style.colors` với semantic keys (front, body, handle, metal, stone...) override token palette. 8 palette (thêm white-oak, navy-brass, green-sage, grey-minimal) + token mới (metal, fabric, stone, ceramic, plantGreen, ledWarm, accent2). |
| Iso renderer shading + bo góc 3D | Done | `src/renderers/iso-renderer.js` | `shadeFaceColor` luminance theo mặt (top 1.1 → bottom 0.78); `drawRoundedBox` vẽ mặt front/top với góc bo quadratic-curve; cylinder có end-ellipse + highlight line. |
| Expression `===`/`!==` + shape error isolation | Done | `src/template-engine/expression.js`, `src/template-engine/interpreter.js` | Tokenizer alias `===`→`==`, `!==`→`!=`. `renderTemplate` try/catch từng shape — shape lỗi bị skip + push `_validationWarnings`, không crash view. Ternary vẫn không hỗ trợ: dùng 2 shape với `if`. |
| Dynamic AI catalog prompt từ DB | Done | backend `utils/interiorCatalogPrompt.js`, `routes/interior.js` | Catalog section sinh từ `InteriorTemplate` (seed+approved), cache 5 phút, rank theo message keywords, max 60 rows (agent 40). Dùng cho /chat apply, proposal, agent initial + system prompt. Static `INTERIOR_CATALOG_VI/EN` chỉ còn là fallback/analyze. |
| Auto-seed templates + workshop | Done | backend `utils/interiorTemplateAssets.js`, `server/index.js`, `scripts/{seed-interior-templates,sync-interior-assets}.mjs`, `server/assets/interior/*` | Server start upsert 14 builtin (status seed) + 42 workshop components (status approved), có normalize `side`→left/right và token alias, validate qua `templateValidator`. Đường dẫn assets có fallback: ưu tiên `tools/` (dev), fallback `server/assets/interior/` (bundle cho Fly image, sinh bởi `npm run sync:interior-assets` — chạy lại trước khi deploy nếu assets đổi). Skills agent cũng fallback tương tự. |
| Geometry validation + repair loop | Done | backend `utils/interiorModelGeometry.js`, `routes/interior.js` | Sau AI output: check occupied-length vs model dims, out-of-bounds, overlap từng cặp, z tủ trên/tủ dưới → warnings (không block). Có warnings → 1 vòng repair gọi lại AI với danh sách lỗi. Template thiếu dims → fill từ `params.default` (DB definitions + engine normalizeItems). |
| Validation panel UI | Done | `src/ui/main-renderer.js` (`renderValidationPanel`), i18n | Hiển thị validate errors + `_validationWarnings` + `reviewModel()` issues thành panel cảnh báo trên các tab. |

---

## 5. Known Issues & TODOs

> **2026-07-02:** Xem `IMPROVEMENT_PLAN.md` (project root) — phân tích 26 phát hiện (F1-F26) và roadmap 5 phase (A-E). **2026-07-03: toàn bộ Phase A-E đã được triển khai** (xem feature table); các mục dưới đây là kết quả review sau triển khai.

### High Priority

- [x] **[BUG — crash]** `cab-base-rounded-end` dùng ternary — đã fix: 2 shape với `if`, tokenizer hỗ trợ `===`/`!==`, `renderTemplate` bọc try/catch skip shape lỗi + warning.
- [x] **[BUG]** `resolveItemBoxes` bỏ qua `item.color` — đã fix: raw box có `color` derive bộ faces qua `colorFaces()` (shade theo mặt).
- [x] Prompt catalog backend — đã thay bằng catalog động từ DB (`utils/interiorCatalogPrompt.js`, cache 5 phút, rank theo message) dùng cho /chat, proposal, agent.
- [x] **[DEPLOY GAP]** Auto-seed thiếu assets trên Fly — đã fix: `npm run sync:interior-assets` (backend) bundle templates/workshop/skills vào `server/assets/interior/`; runtime ưu tiên `tools/` rồi fallback bundle. **Nhắc: chạy lại sync + redeploy backend khi đổi template/component/skill.**
- [x] **[BUG — coordinate convention]** Đã thống nhất: engine `resolveRunCoord` (cả backend mirror trong `interiorModelGeometry.js`) dùng `x` = vị trí dọc trục run cho MỌI hướng, `z` = offset vuông góc từ tường — khớp prompt. Few-shot chữ L viết lại (return run `south` chứa corner, main run origin lệch qua khối góc) — verified 0 warnings + in-bounds. Re-normalize giữ nguyên world coords (early-return khi item có `_runDirection`).
- [x] Geometry check "occupied length" — model đa run chỉ cảnh báo overshoot; two-sided check giữ cho bố cục thẳng 1 run.
...
- [x] Wire actual image-generation upstream — done: `/generate-render` gọi Gemini image model, fallback conditioning URL khi lỗi/thiếu key.
